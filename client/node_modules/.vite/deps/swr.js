...
var preload = (key_, fetcher) => {
	if (IS_SERVER) return;
	const [key, fnArg] = serialize$1(key_);
...
*/ var useSWRHandler = (_key, fetcher, config) => {
	...
	if (suspense) {
		...
		const req = PRELOAD[key];
		use(!isUndefined$1(req) && hasKeyButNoData ? boundMutate(req) : resolvedUndef);
		if (!isUndefined$1(error) && hasKeyButNoData) throw error;
...
//#endregion
export { SWRConfig, useSWR as default, mutate, preload, unstable_serialize, useSWRConfig };
