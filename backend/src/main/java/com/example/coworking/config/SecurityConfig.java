...
public class SecurityConfig {
  ...
  public SecurityFilterChain securityFilterChain(HttpSecurity http,
      com.example.coworking.config.JwtAuthenticationFilter jwtAuthenticationFilter) {
    return http.cors(Customizer.withDefaults()).csrf(AbstractHttpConfigurer::disable)
        ...
        .authorizeHttpRequests(auth -> auth.requestMatchers(HttpMethod.OPTIONS, "/**").permitAll()
            ...
            .hasAnyRole("ADMIN", "USER").requestMatchers(HttpMethod.GET, "/api/bookings/**")
            .hasAnyRole("ADMIN", "USER").requestMatchers(HttpMethod.POST, "/api/bookings/*/cancel")
            .hasAnyRole("ADMIN", "USER").requestMatchers(HttpMethod.POST, "/api/payments")
            .hasAnyRole("ADMIN", "USER")
            .requestMatchers(HttpMethod.GET, "/api/payments/check/booking/**")
            .hasAnyRole("ADMIN", "USER").requestMatchers("/api/users/**").permitAll()
  ...
  public CorsConfigurationSource corsConfigurationSource() {
    List<String> originPatterns = Arrays.stream(corsAllowedOriginPatterns.split(","))
        .map(String::trim).filter(pattern -> !pattern.isEmpty()).toList();

    CorsConfiguration configuration = new CorsConfiguration();
    configuration.setAllowedOriginPatterns(
        originPatterns.isEmpty() ? List.of("*") : originPatterns);
  ...
  public UserDetailsService userDetailsService() {
    return email -> userRepository.findByEmail(email).map(user -> {
      ...
      return org.springframework.security.core.userdetails.User.withUsername(user.getEmail())
          .password(user.getPassword()).authorities(role.getAuthority()).build();
